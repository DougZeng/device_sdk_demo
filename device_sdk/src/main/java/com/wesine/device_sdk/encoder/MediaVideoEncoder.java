...
public class MediaVideoEncoder extends MediaEncoder {
    ...
    protected void prepare() throws IOException {
        ...
        Logger.d("selected codec: %s", videoCodecInfo.getName());
        ...
        for (int i = 0; i < numCodecs; i++) {
            ...
            for (int j = 0; j < types.length; j++) {
                if (types[j].equalsIgnoreCase(mimeType)) {
                    Logger.d("codec: %s ,MIME = %s", codecInfo.getName(), types[j]);
    ...
    protected static final int selectColorFormat(final MediaCodecInfo codecInfo, final String mimeType) {
        ...
        if (result == 0)
            Logger.d("couldn't find a good color format for %s  %s", codecInfo.getName(), mimeType);
        return result;
    }
...
//        	MediaCodecInfo.CodecCapabilities.COLOR_QCOM_FormatYUV420SemiPlanar,
    ...
    private static final boolean isRecognizedViewoFormat(final int colorFormat) {
        Logger.d("isRecognizedViewoFormat:colorFormat= %d" + colorFormat);
